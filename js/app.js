const stories = [
  {
    title: 'Croco el cocodrilo',
    icon: '🐊',
    video: 'videos/croco.mp4',
    questions: [
  ...
  {
    title: 'El sol saltarín de Alba y Mateo',
    icon: '🐰',
    video: 'videos/alba-mateo.mp4',
    questions: [
...
const games = {
  ...
  letterA: {
    title: 'Animales que empiezan con A',
    emoji:'🔤',
    questions: [
...
function openGamesMenu() {
  stopStoryVideo();
  hideAll();
  $('screenGamesMenu').classList.remove('hidden');
}
...
function startApp() {
  ...
  questionIndex = 0;
  selectedAnswer = null;
  scores = stories.map(() => ({ correct: 0, total: 0, texts: [] }));
  showVideo();
}
  ...
  if (q.type === 'choice') {
    q.options.forEach((op, i) => {
      ...
      label.innerHTML = `
        ...
        <span>${String.fromCharCode(97 + i)}) ${op}</span>
      `;
...
function checkAnswer() {
  const q = stories[storyIndex].questions[questionIndex];
  ...
  if (q.type === 'choice') {
    scores[storyIndex].total++;
  }

  if (selectedAnswer === q.correct) {
    scores[storyIndex].correct++;
    showFeedback(true);
  } else {
    showFeedback(false);
  }
...
function continueFromText() {
  const val = ($('textResponse')?.value || '').trim();

  scores[storyIndex].texts.push({
    question: stories[storyIndex].questions[questionIndex].text,
    answer: val
  });
...
function showFeedback(ok) {
  ...
  if (ok) {
    ...
    $('feedbackText').textContent = '¡Muy bien! Respondiste correctamente.';
    ...
    $('feedbackText').textContent = 'Ups... esa no es la respuesta. Inténtalo otra vez.';
...
function nextStep() {
  ...
  if (questionIndex < stories[storyIndex].questions.length) {
    showQuestion();
  } else {
    endStory();
  }
...
function endStory() {
  ...
  const sc = scores[storyIndex];

  $('storyScore').textContent =
    `Terminaste ${stories[storyIndex].title}. Respuestas correctas: ${sc.correct} de ${sc.total}.`;

  document.querySelector('#screenStoryEnd .btn').textContent =
    storyIndex < stories.length - 1 ? 'Ir al siguiente cuento' : 'Ver resultado final';

  playSound('soundClap');
  confetti();

  $('screenStoryEnd').classList.remove('hidden');
}
...
function showFinal() {
  ...
  stories.forEach((s, i) => {
    totalC += scores[i].correct;
    totalT += scores[i].total;

    const row = document.createElement('div');
    row.className = 'result-row';
    row.innerHTML = `<strong>${s.icon} ${s.title}</strong><span>${scores[i].correct}/${scores[i].total}</span>`;
  ...
  const row = document.createElement('div');
  row.className = 'result-row';
  row.innerHTML = `<strong>⭐ Total</strong><span>${totalC}/${totalT}</span>`;
  $('finalResults').appendChild(row);
...
function showGameQuestion() {
  ...
  const q = g.questions[gameIndex];

  $('gameProgress').textContent = `Pregunta ${gameIndex + 1} de ${g.questions.length}`;
  $('gameEmoji').textContent = g.emoji;
  ...
  q.options.forEach((op, i) => {
    ...
    btn.innerHTML = `<span>${op.value}</span>`;
    btn.setAttribute('aria-label', op.label);
    btn.onclick = () => checkGameAnswer(i);
...
function checkGameAnswer(i) {
  hideAll();

  const q = games[currentGame].questions[gameIndex];

  if (i === q.correct) {
    gameScore++;
    $('gameFeedbackEmoji').textContent = '🎉😊';
    $('gameFeedbackTitle').textContent = '¡FELICIDADES!';
    $('gameFeedbackText').textContent = '¡Muy bien!';
    ...
    $('gameFeedbackEmoji').textContent = '😢';
    $('gameFeedbackTitle').textContent = 'Incorrecto';
    $('gameFeedbackText').textContent = 'Ups... inténtalo otra vez.';
    $('gameNextBtn').classList.add('hidden');
    $('gameRetryBtn').classList.remove('hidden');
    playSound('soundWrong');
  }

  $('screenGameFeedback').classList.remove('hidden');
}

function retryGameQuestion() {
  showGameQuestion();
}
...
function nextGameQuestion() {
  ...
  if (gameIndex < games[currentGame].questions.length) {
    showGameQuestion();
  } else {
    hideAll();

    $('gameFeedbackEmoji').textContent = '🏆';
    $('gameFeedbackTitle').textContent = '¡Juego terminado!';
    $('gameFeedbackText').textContent = `Lograste ${gameScore} de ${games[currentGame].questions.length}.`;

    $('gameNextBtn').textContent = 'Volver a juegos';
    $('gameNextBtn').onclick = () => {
      $('gameNextBtn').textContent = 'Siguiente';
